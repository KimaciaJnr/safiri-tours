...
import { MapPin, Clock, Users, Home, Calendar, CheckCircle2, XCircle, ChevronRight } from 'lucide-react';
...
export default function SafariDetailPage({ slug }) {
  ...
  return (
    <div className="min-h-screen bg-cream text-ink">
      ...
      <main id="main" tabIndex={-1} className="mx-auto max-w-5xl px-6 py-12 lg:px-10">
        {/* Breadcrumb */}
        ...
        <header className="mb-10 overflow-hidden rounded-[2rem] border border-line bg-white shadow-[0_18px_45px_rgba(17,24,39,0.04)]">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 border-t border-cream-sand bg-cream-light px-6 py-4 md:px-10">
            ...
            <div className="flex items-center gap-4">
              ...
              <a
                href={`#inquiry?package=${encodeURIComponent(pkg.title)}`}
        ...
        {pkg.itinerary && pkg.itinerary.length > 0 && (
          <section className="mb-10">
            <h2 className="mb-6 text-2xl font-bold text-ink">Safari Itinerary</h2>
            <div className="space-y-6">
              {pkg.itinerary.map((day) => (
                <div
                  key={day.day}
                  className="relative rounded-[1.5rem] border border-cream-sand bg-white p-6 shadow-sm"
                >
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      <h3 className="text-lg font-bold text-ink">Day {day.day}: {day.title}</h3>
                      <p className="mt-2 text-sm leading-relaxed text-gray-700">{day.description}</p>
                      {day.mainActivity && (
                        <span className="mt-3 inline-block rounded-full bg-cream-soft px-3 py-1 text-xs font-semibold text-teal">
                          {day.mainActivity}
                        </span>
                      )}
                    </div>
        ...
        {pkg.highlights && pkg.highlights.length > 0 && (
          <section className="mb-10 rounded-[1.75rem] bg-teal p-6 md:p-8 text-white shadow-[0_18px_45px_rgba(15,91,91,0.12)]">
        ...
        {/* CTA Section */}
        <section className="mb-10 rounded-[2rem] bg-cream-soft border border-cream-sand p-8 md:p-10 text-center">
